# APIHub worker image.
#
# Structurally identical to the API image, differing only in the entry point.
# The worker is a separate image because it scales on a different axis: health
# probes are IO-bound and independent of request traffic (report section 9).
...
COPY . .
RUN pnpm --filter @apihub/worker build
...
COPY --from=build --chown=apihub:apihub /repo/apps/worker/dist ./dist
...
COPY --from=build --chown=apihub:apihub /repo/apps/worker/node_modules ./node_modules

USER apihub
# The worker serves no HTTP, so there is nothing to probe. Liveness is the
# process itself; the orchestrator restarts it if it exits.
